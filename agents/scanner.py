...
class ScannerAgent(BaseAgent):
    ...
    def __init__(self, llm_client: Optional[LLMClient] = None, slither_path: str = "slither"):
    ...
    async def process(
        ...
        try:
            ...
            print(f"[Scanner] Slither error: {e}")
            slither_findings = []
        ...
        try:
            ...
            if result.stdout:
                try:
                    ...
                    pass  # Slither may output errors to stdout

        except subprocess.TimeoutExpired:
            print("[Scanner] Slither timeout")
        except FileNotFoundError:
            print("[Scanner] Slither not found, skipping static analysis")
        except Exception as e:
            print(f"[Scanner] Slither error: {e}")
        finally:
            # Cleanup temp file
    ...
    def _extract_line_from_slither(self, detector: Dict[str, Any]) -> Optional[int]:
        ...
        if elements:
            # Get first element's line number
    ...
    async def _llm_scan(self, source_code: str) -> List[Finding]:
        ...
        try:
            ...
            print(f"[Scanner] LLM parse error: {e}")
            return []
    ...
    def _merge_findings(
        ...
        # Use issue_type + approximate line number for deduplication
        seen = set()
        merged = []

        for finding in slither_findings + llm_findings:
            # Create dedup key (issue type + line range)
            line_range = range(
...
def quick_pattern_scan(source_code: str) -> List[Finding]:
    ...
    for i, line in enumerate(lines):
        if ("call{" in line or ".call(" in line or ".send(" in line) and "require" not in line:
            # Check if previous line has variable assignment
